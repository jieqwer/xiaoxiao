var mizhu = new function() {
	...
	function messageBox(html, title, message, type) {
		var jq = $(html);
		if(type == "toast") {
			jq.find(".window-panel").width(message.length * 10).css("margin-left", -message.length * 20 / 2).css("margin-top", -mizhu.height / 4);
		} else {
			jq.find(".window-panel").width(mizhu.width-396).css("margin-left", (-mizhu.width+396) / 2).css("margin-top", -mizhu.height / 2 - 140);
		}
		if(valempty(title)) {
